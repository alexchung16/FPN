...
def proposal_target_layer(rpn_rois, gt_boxes):
    ...
    labels, rois, bbox_targets = sample_rois(all_rois, gt_boxes, fg_rois_per_image,
...
def sample_rois(all_rois, gt_boxes, fg_rois_per_image,
    ...
    if bg_inds.size > 0:
        bg_inds = np.random.choice(bg_inds, size=int(bg_rois_per_this_image), replace=False)
    ...
    bbox_target_data = compute_targets( rois, gt_boxes[gt_assignment[keep_inds], :-1], labels)

    bbox_targets = get_bbox_regression_labels(bbox_target_data, num_classes)
